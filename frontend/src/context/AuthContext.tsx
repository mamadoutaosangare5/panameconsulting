...
function getRememberMeCookie(): boolean {
	...
	return document.cookie.split("; ").some((row) => row.startsWith("remember_me="));
}
...
function deleteRememberMeCookie(): void {
	document.cookie = `${encodeURIComponent("remember_me")}=` + `; path=/` + `; max-age=0`;
}
...
export async function apiFetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
	...
	try {
		const refreshRes = await fetch(`${API_URL}/auth/refresh`, {
			...
			body: JSON.stringify(_refreshTokenValue ? { refresh_token: _refreshTokenValue } : {}),
		});
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
	...
	const refreshUserProfile = useCallback(async (): Promise<void> => {
		try {
			...
			const response = await apiFetch(`${API_URL}/user/profile?t=${timestamp}`, {
				method: "GET",
			});
			...
			const message = error instanceof Error ? error.message : "Erreur lors du rafraîchissement du profil";
			toast.error(message);
			throw error;
		}
	}, []);
	...
	const refreshToken = useCallback(async (): Promise<void> => {
		...
		try {
			const response = await fetch(`${API_URL}/auth/refresh`, {
				...
				credentials: "include",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify(refreshTokenRef.current ? { refresh_token: refreshTokenRef.current } : {}),
			});
	...
	const login = async (email: string, password: string, rememberMe = false): Promise<void> => {
		try {
			...
			const body: BackendDTO_ApiResponse<BackendDTO_LoginData> = await response.json();
	...
	const register = async (data: {
		...
		try {
			...
			const body: BackendDTO_ApiResponse<BackendDTO_RegisterData> = await response.json();
	...
	const logoutAll = async (): Promise<void> => {
		try {
			...
			const body: BackendDTO_ApiResponse<BackendDTO_LogoutAllData> = await response.json();
			...
			toast.success(`${body.data.sessionsTerminated} session(s) terminée(s) (admin épargné)`);
		} catch (error) {
			const message = error instanceof Error ? error.message : "Erreur lors de la déconnexion globale";
			toast.error(message);
			throw error;
		}
	};
	...
	const forgotPassword = async (email: string): Promise<void> => {
		try {
			...
			const body: BackendDTO_ApiResponse<BackendDTO_ForgotPasswordData> = await response.json();

			if (!response.ok) {
				const msg = body.message || "Erreur lors de la demande de réinitialisation";
				toast.error(msg);
				throw new Error(msg);
			}
	...
	const resetPassword = async (token: string, newPassword: string): Promise<void> => {
		try {
			...
			const body: BackendDTO_ApiResponse<BackendDTO_ResetPasswordData> = await response.json();
	...
	const changePassword = async (oldPassword: string, newPassword: string): Promise<void> => {
		try {
			...
			const body: BackendDTO_ApiResponse<BackendDTO_ChangePasswordData> = await response.json();
