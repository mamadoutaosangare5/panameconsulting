...
export const useRendezvous = (options: UseRendezvousOptions = {}): UseRendezvousReturn => {
	const { autoLoad = true, initialParams = {}, initialStartDate, initialEndDate, refreshInterval } = options;
	...
	const loadRendezvous = useCallback(
		async (params: RendezvousQueryDto = {}) => {
			if (!isAdmin) {
				console.warn("[useRendezvous] Tentative d'accès admin par utilisateur non-admin");
				return;
			}
	 ...
	 * Charge les dates disponibles
	 */
	const loadAvailableDates = useCallback(async (startDate?: string, endDate?: string) => {
		setLoadingKey("dates", true);

		try {
			const dates = await rendezvousService.getAvailableDates(startDate, endDate);
			setAvailableDates(dates);
			console.log(`[useRendezvous] ✅ ${dates.length} dates disponibles chargées`);
		} catch (err) {
	 ...
	 */
	const getAvailableSlots = useCallback(async (date: string): Promise<AvailableSlotsDto> => {
		...
		try {
			...
			console.log(`[useRendezvous] ✅ ${slots.availableSlots.length} créneaux pour ${date}`);
			return slots;
		} catch (err) {
			console.error(`[useRendezvous] Erreur chargement créneaux pour ${date}:`, err);
	...
	const getRendezvousByDate = useCallback(
		async (date: string): Promise<RendezvousResponseDto[]> => {
			...
			try {
				...
				const response = await apiFetch(`${API_URL}/rendezvous/by-date/${date}`, {
					method: "GET",
				});

				if (!response.ok) {
					const errorData = await response.json().catch(() => ({}));
					const errorMessage = errorData.message || "Erreur lors de la récupération des rendez-vous";
					toast.error(errorMessage);
					return [];
				}

				const data = await response.json();
				console.log(`[useRendezvous] ✅ ${data.length} rendez-vous pour le ${date}`);
				return Array.isArray(data) ? data : [];
			} catch (err) {
				console.error(`[useRendezvous] Erreur chargement rendez-vous pour ${date}:`, err);
				return [];
			}
		},
		[isAdmin],
	...
	const completeRendezvous = useCallback(
		async (id: string, data: CompleteRendezvousDto): Promise<RendezvousResponseDto | null> => {
			...
			try {
				...
				const avisMsg = data.avisAdmin === "FAVORABLE" ? "favorable" : "défavorable";
				toast.success(`Rendez-vous terminé avec avis ${avisMsg}`);
				console.log(`[useRendezvous] ✅ Rendez-vous ${id} terminé (avis: ${data.avisAdmin})`);
	...
	const refreshTodayRendezvous = useCallback(async () => {
		...
		setRendezvous((prev) => [...prev.filter((r) => r.date !== today), ...todayRdv]);

		console.log(`[useRendezvous] 🔄 Rendez-vous du jour rafraîchis (${todayRdv.length})`);
	...
	const getUpcomingRendezvous = useCallback(
		async (limit = 10): Promise<RendezvousResponseDto[]> => {
			...
			try {
				...
				console.log(`[useRendezvous] ✅ ${upcoming.length} prochains rendez-vous`);
				return upcoming;
			} catch (err) {
				console.error("[useRendezvous] Erreur chargement prochains rendez-vous:", err);
				return [];
			}
		},
		[isAdmin],
	...
	const checkAvailability = useCallback(
		async (date: string, time: TimeSlot): Promise<AvailabilityCheckDto | null> => {
			...
			try {
				...
				console.log(`[useRendezvous] ✅ Disponibilité ${date} ${time}: ${result.available ? "oui" : "non"}`);
				return result;
			} catch (err) {
				console.error("[useRendezvous] Erreur vérification disponibilité:", err);
				return null;
			} finally {
	...
	const setFilters = useCallback(
		(newFilters: RendezvousFilters) => {
			...
			rendezvousService.searchRendezvous(newFilters, 1, pagination.limit).then((res) => {
				setRendezvous(res.data);
				setPagination({
					total: res.total,
					page: res.page,
	...
	const nextPage = useCallback(() => {
		if (pagination.hasNext) {
			loadRendezvous({ ...initialParamsRef.current, page: pagination.page + 1 });
		}
	...
	}, [autoLoad, isAuthenticated, isAdmin, loadRendezvous, loadStatistics, loadAvailableDates]);
