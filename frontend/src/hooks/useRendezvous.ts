...
export const useRendezvous = (
  ...
  const loadRendezvous = useCallback(
    async (params: RendezvousQueryDto = {}) => {
      ...
      try {
        ...
        const res = await rendezvousService.getAllRendezvous(mergedParams);
        ...
        console.log(`[useRendezvous] ✅ ${res.data.length} rendez-vous chargés (page ${res.page}/${res.totalPages})`);
      } catch (err) {
  ...
  const getRendezvousByDate = useCallback(
    async (date: string): Promise<RendezvousResponseDto[]> => {
      ...
      try {
        const data = await rendezvousService.getRendezvousByDate(date);
  ...
  const cancelRendezvous = useCallback(
    async (id: string, data: CancelRendezvousDto): Promise<RendezvousResponseDto | null> => {
      ...
      try {
        ...
        toast.info("Rendez-vous annulé avec succès");
        console.log(`[useRendezvous] ✅ Rendez-vous ${id} annulé`);
