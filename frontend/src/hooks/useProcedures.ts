...
export interface UseProceduresActions {
	...
	setFilters: (f: ProcedureFilters | ((prev: ProcedureFilters) => ProcedureFilters)) => void;
	...
	cancelProcedure: (id: string, reason?: string) => Promise<boolean>;
...
export function useProcedures(options: UseProceduresOptions = {}): UseProceduresState & UseProceduresActions {
	const {
		autoLoad = true,
	...
	const [filters, setFiltersState] = useState<ProcedureFilters>(DEFAULT_FILTERS);
	...
	// ── Helpers état ─────────────────────────────────────────────────────────
	const setLoad = (k: keyof ProcedureLoadingState, v: boolean) => setLoading((prev) => ({ ...prev, [k]: v }));
	...
	const update = useCallback(
		async (id: string, data: UpdateProcedureDto): Promise<ProcedureResponseDto | null> => {
			...
			try {
				...
				// Rollback
				if (original) setProcedures((prev) => prev.map((p) => (p.id === id ? original : p)));
	...
	const updateStep = useCallback(
		async (id: string, stepName: StepName, data: UpdateStepDto): Promise<ProcedureResponseDto | null> => {
			...
			try {
				...
				if (original) setProcedures((prev) => prev.map((p) => (p.id === id ? original : p)));
				setError((err as Error).message ?? "Erreur lors de la mise à jour de l'étape");
	...
	const findByRendezvousId = useCallback(
		async (rendezVousId: string): Promise<ProcedureResponseDto | null> => {
			try {
				...
				const procedure = await ProceduresService.findByRendezvousId(rendezVousId);
	...
	const setPage = useCallback((page: number) => setQueryState((prev) => ({ ...prev, page })), []);
	const setLimit = useCallback((limit: number) => setQueryState((prev) => ({ ...prev, limit, page: 1 })), []);
	...
	const cancelProcedure = useCallback(
		async (id: string, reason?: string): Promise<boolean> => {
			try {
				...
				setProcedures((prev) => prev.map((p) => (p.id === id ? { ...p, statut: "CANCELLED" as any } : p)));

				if (selectedProcedure?.id === id) {
					setSelectedProcedure(null);
				}

				log("cancelProcedure OK", { id });
				return true;
			} catch (err: unknown) {
				setError(err instanceof Error ? err.message : "Erreur lors de l'annulation");
				log("cancelProcedure ERR", err);
				return false;
			} finally {
				setLoad("delete", false);
			}
		},
		[log, selectedProcedure],
	);

	// ─────────────────────────────────────────────────────────────────────────
	// Validation — délégué au service
	// ─────────────────────────────────────────────────────────────────────────
	const validate = useCallback((data: Partial<CreateProcedureDto>) => ProceduresService.validate(data), []);
	const isValid = useCallback((data: Partial<CreateProcedureDto>) => ProceduresService.isValid(data), []);
