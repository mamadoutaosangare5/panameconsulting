...
export const useDestinations = () => {
	...
	const loadDestinations = useCallback(async () => {
		...
		setError(null);
		try {
			console.log("[useDestinations] Calling destinationsService.getAllDestinations()");
			...
			console.log("[useDestinations] Type of data:", typeof data, "Is array:", Array.isArray(data)); // Debug
	...
	const createDestination = useCallback(
		async (data: CreateDestinationData) => {
			...
			setError(null);
			try {
				const newDestination = await destinationsService.createDestination(data);
	...
	const updateDestination = useCallback(
		async (id: string, data: UpdateDestinationData) => {
			...
			setError(null);
			try {
				const updatedDestination = await destinationsService.updateDestination(id, data);
				// Rafraîchir la liste pour s'assurer d'avoir les données à jour
				await loadDestinations();
	...
	const cleanupImages = useCallback(async () => {
		...
		try {
			...
			toast.success(`${result.deletedFiles.length} images orphelines supprimées`);
			return result;
		} catch (err) {
			const message = err instanceof Error ? err.message : "Erreur inconnue";
			setError(message);
