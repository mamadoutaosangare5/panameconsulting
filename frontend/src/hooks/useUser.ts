...
export interface UseUserReturn {
	...
	updateUserStatus(id: string, params: UpdateUserStatusParams): Promise<AppUser | null>;
...
export function useUser(): UseUserReturn {
	...
	const [error, setError] = useState<Partial<Record<keyof LoadingState, string | null>>>({});
	...
	// PATCH /admin/profile  (sans email)
	// ─────────────────────────────────────────────────────────
	const patchAdminProfile = useCallback(async (params: UpdateAdminProfileParams): Promise<AppUser | null> => {
		...
		try {
			...
			const msg = err instanceof Error ? err.message : "Erreur mise à jour profil admin";
	...
	// GET /admin/users/all
	// ─────────────────────────────────────────────────────────
	const fetchUsers = useCallback(async (params: GetUsersParams = {}): Promise<void> => {
		...
		try {
			...
			const msg = err instanceof Error ? err.message : "Erreur récupération utilisateurs";
			setErr("list", msg);
			toast.error(msg);
		} finally {
			stopLoading("list");
	...
	const fetchUserById = useCallback(async (id: string): Promise<AppUser | null> => {
		...
		try {
			...
			const msg = err instanceof Error ? err.message : "Utilisateur introuvable";
	...
	// POST /admin/users/create
	// ─────────────────────────────────────────────────────────
	const createNewUser = useCallback(async (params: CreateUserParams): Promise<AppUser | null> => {
		startLoading("create");
		try {
	...
	// PATCH /admin/user/:id
	// ─────────────────────────────────────────────────────────
	const patchUser = useCallback(async (id: string, params: UpdateUserParams): Promise<AppUser | null> => {
		startLoading("update");
		try {
			...
			const msg = err instanceof Error ? err.message : "Erreur mise à jour utilisateur";
			setErr("update", msg);
			toast.error(msg);
			return null;
		} finally {
	...
	const removeUser = useCallback(async (id: string): Promise<boolean> => {
		...
		try {
			...
			const msg = err instanceof Error ? err.message : "Erreur suppression utilisateur";
			setErr("delete", msg);
			toast.error(msg);
			return false;
		} finally {
	...
	const updateUserStatusHook = useCallback(
		async (id: string, params: UpdateUserStatusParams): Promise<AppUser | null> => {
			// Protection : empêcher la désactivation des administrateurs
			if (!params.isActive) {
				// Vérifier si l'utilisateur est un admin avant de continuer
				const currentUser = userList?.items.find((u) => u.id === id) || selectedUser;
			...
			try {
				...
				toast.success(params.isActive ? "Utilisateur activé avec succès" : "Utilisateur désactivé avec succès");
				return updated;
			} catch (err) {
