...
async function handleResponse<T>(res: Response): Promise<T> {
	...
	if (!res.ok) {
		...
		const err = new Error(apiError.message || `Erreur ${res.status}`) as Error & {
			apiError: ApiError;
			status: number;
		};
...
export const ProceduresService = {
	...
	async getStatistics(): Promise<ProcedureStatisticsDto> {
		try {
			...
			const result = handleResponse<ProcedureStatisticsDto>(res);
	...
	async updateStep(id: string, stepName: StepName, data: UpdateStepDto): Promise<ProcedureResponseDto> {
		try {
			const res = await apiFetch(`${BASE_URL}/admin/procedures/${id}/steps/${stepName}`, {
				method: "PATCH",
				headers: JSON_HEADERS,
				body: JSON.stringify(data),
			});
	...
	async addStep(id: string, stepName: StepName): Promise<ProcedureResponseDto> {
		try {
			const res = await apiFetch(`${BASE_URL}/admin/procedures/${id}/steps/${stepName}`, { method: "POST" });
			const result = handleResponse<ProcedureResponseDto>(res);
	...
	async findByEmail(email: string): Promise<ProcedureResponseDto[]> {
		const res = await apiFetch(`${BASE_URL}/procedures/${encodeURIComponent(email)}`, { method: "GET" });
		...
		if (!res.ok) {
			const err = new Error((body as { message?: string })?.message ?? `Erreur ${res.status}`) as Error & {
				apiError: unknown;
			};
			err.apiError = body;
			throw err;
		}
		return Array.isArray(body) ? body : (((body as { data?: unknown })?.data as ProcedureResponseDto[]) ?? []);
	},
	 ...
	 */
	async findWithFilters(filters: ProcedureFilters): Promise<PaginatedProcedureResponseDto> {
	...
	validate(data: Partial<CreateProcedureDto>): Record<string, string> {
		...
		if ("rendezVousId" in data) {
			if (!data.rendezVousId) errors.rendezVousId = "L'ID du rendez-vous est requis";
			else if (!UUID_RE.test(data.rendezVousId)) errors.rendezVousId = "UUID invalide";
		}
		if ("prenom" in data) {
			if (!data.prenom || data.prenom.trim().length < 2) errors.prenom = "Min 2 caractères";
			else if (data.prenom.length > 50) errors.prenom = "Max 50 caractères";
			else if (!NAME_RE.test(data.prenom)) errors.prenom = "Caractères invalides";
		}
		if ("nom" in data) {
			if (!data.nom || data.nom.trim().length < 2) errors.nom = "Min 2 caractères";
		...
		if ("email" in data) {
			...
			else if (!EMAIL_RE.test(data.email)) errors.email = "Format d'email invalide";
		}
		if ("telephone" in data) {
			if (!data.telephone) errors.telephone = "Le téléphone est requis";
			else if (!PHONE_RE.test(data.telephone)) errors.telephone = "Format international requis";
		}
		if ("destination" in data && (!data.destination || data.destination.trim().length < 2))
		...
		if ("niveauEtude" in data && !data.niveauEtude?.trim()) errors.niveauEtude = "Niveau d'étude requis";
