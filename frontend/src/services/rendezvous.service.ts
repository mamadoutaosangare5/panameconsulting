...
class RendezvousService {
	 ...
	 */
	private buildUrl(path: string, params?: Record<string, string | number | boolean> | RendezvousQueryDto): string {
	 ...
	 */
	async getAvailableDates(startDate?: Date | string, endDate?: Date | string): Promise<AvailableDatesResponseDto[]> {
	 ...
	 */
	async checkAvailability(date: Date | string, time: TimeSlot): Promise<AvailabilityCheckDto> {
		const dateStr = this.formatDate(date);
	 ...
	 */
	async createRendezvous(data: CreateRendezvousDto): Promise<RendezvousResponseDto> {
	 ...
	 */
	async cancelRendezvous(id: string, data: CancelRendezvousDto): Promise<RendezvousResponseDto> {
	 ...
	 */
	async updateRendezvous(id: string, data: UpdateRendezvousDto): Promise<RendezvousResponseDto> {
		// Valider les champs "Autre" obligatoires
	 ...
	 */
	private getEffectiveValue(mainValue: string, autreValue?: string | null): string {
	...
	private prepareCreateData(data: CreateRendezvousDto): CreateRendezvousDto {
		return {
			...data,
			destination: this.getEffectiveValue(data.destination, data.destinationAutre),
			niveauEtude: this.getEffectiveValue(data.niveauEtude, data.niveauEtudeAutre),
	...
	private prepareUpdateData(data: UpdateRendezvousDto): UpdateRendezvousDto {
		...
		if (data.destination !== undefined) {
			prepared.destination = this.getEffectiveValue(data.destination, data.destinationAutre);
		}
		if (data.niveauEtude !== undefined) {
			prepared.niveauEtude = this.getEffectiveValue(data.niveauEtude, data.niveauEtudeAutre);
		}
		if (data.filiere !== undefined) {
			prepared.filiere = this.getEffectiveValue(data.filiere, data.filiereAutre);
		}
	 ...
	 */
	async canUserCreateRendezvous(email: string, date: Date | string): Promise<boolean> {
		try {
			...
			const existingForDate = userRendezvous.some(
				(rdv) => rdv.date === dateStr && (rdv.status === "CONFIRMED" || rdv.status === "PENDING"),
			);
	 ...
	 */
	static getSuggestedOptions(type: "destination" | "niveauEtude" | "filiere"): string[] {
		switch (type) {
			case "destination":
				return ["France", "Russie", "Chypre", "Chine", "Maroc", "Algérie", "Turquie", "Autre"];
			case "niveauEtude":
				return ["Bac", "Bac+1", "Bac+2", "Licence", "Master I", "Master II", "Doctorat", "Autre"];
			case "filiere":
				return ["Informatique", "Médecine", "Droit", "Commerce", "Ingénierie", "Architecture", "Autre"];
			default:
				return [];
		}
	}
