...
export const MessagesService = {
	...
	async findAll(query: ContactQueryDto = {}): Promise<ContactListResponse> {
		try {
			...
			if (query.isReplied !== undefined) params.set("isReplied", String(query.isReplied));
	...
	async getStatistics(): Promise<ContactStatistics> {
		try {
			...
			console.error("[message.service] Erreur récupération statistiques:", error);
			throw error;
		}
	},
	...
	async getUnreadCount(): Promise<{ count: number }> {
		try {
			...
			console.error("[message.service] Erreur récupération count unread:", error);
			throw error;
		}
	},
	...
	async respond(id: string, payload: RespondContactDto): Promise<ContactResponseDto> {
		try {
	...
	async removePermanent(id: string): Promise<void> {
		try {
			const res = await apiFetch(`${BASE_URL}/admin/contacts/${id}/delete?permanent=true`, {
				method: "DELETE",
			});
			await handleResponse<void>(res);
