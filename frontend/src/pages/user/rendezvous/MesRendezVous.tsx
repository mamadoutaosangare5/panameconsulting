...
const MesRendezvous: React.FC = () => {
  ...
  const fetchRendezvous = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // GET /rendezvous/by-email/:email → Rendezvous[]
      const data: Rendezvous[] = await rendezvousService.getRendezvousByEmail(
        user.email,
      );
  ...
  useEffect(() => {
    ...
    setRendezvous(filtered.slice(start, start + pagination.limit));
  ...
  const handleCancelRendezvous = async (reason?: string) => {
    ...
    try {
      ...
      // PATCH /rendezvous/:id/cancel → Rendezvous (body ignoré par le service backend)
      const updated: Rendezvous = await rendezvousService.cancelRendezvous(
        selectedRdvForCancel.id,
        cancelData,
      );

      // Mettre à jour avec la réponse réelle du backend
      setAllRendezvous((prev) =>
      ...
      if (err instanceof Error) {
        if (err.message.includes("2 heures")) {
          errorMessage =
            "Impossible d'annuler : moins de 2 heures avant le rendez-vous";
        } else if (err.message.includes("terminé")) {
