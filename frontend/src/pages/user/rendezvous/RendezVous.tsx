...
const RendezVous = () => {
			...
			<div className="min-h-screen flex items-center justify-center bg-gray-50">
				<div className="text-center p-8">
					<div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full">
						...
						<p className="text-gray-600 mb-6">Vous devez être connecté pour prendre un rendez-vous.</p>
	...
	const { createRendezvous, checkAvailability, getAvailableSlotsList, getAvailableDates, loading } = useRendezvous({
		autoLoad: false,
	});
	...
	const isStepValid = (step: number): boolean => {
		switch (step) {
			...
			case 2:
				...
				if (formData.niveauEtude === "Autre" && !formData.niveauEtudeAutre?.trim()) return false;
		...
		<div data-aos="fade-up" className="space-y-3">
			...
			<div>
				...
				{showOtherDestination && (
					<div className="mt-3">
						...
						/>
						<p className="mt-1 text-xs text-gray-500">Obligatoire quand "Autre" est sélectionné</p>
					</div>
				)}
			</div>
			...
			<div className="grid gap-3 sm:grid-cols-2">
				...
				<div>
					...
					{showOtherNiveau && (
						<div className="mt-3">
							<label htmlFor="niveauEtudeAutre" className="mb-1 block text-xs font-medium text-gray-700">
								<span className="flex items-center gap-1">
									<Target className="text-sky-500 h-3 w-3" />
				...
				<div>
					...
					{availableSlots.length > 0 ? (
						...
						<div className="rounded border border-amber-300 bg-amber-50 px-3 py-2">
							<p className="text-xs text-amber-700">Aucun créneau disponible pour cette date</p>
						</div>
					)}
					...
					<div key={step} className="flex flex-col items-center">
						<div
							className={`flex h-8 w-8 items-center justify-center rounded-full text-sm transition-all duration-150 ${
								currentStep >= step ? "bg-sky-600 text-white" : "bg-gray-200 text-gray-400"
							}`}
						>
							{step}
						</div>
	...
	const renderSuccessMessage = () => (
		<div data-aos="zoom-in" className="text-center">
			...
			<h2 className="mb-3 text-lg font-bold text-gray-800">Rendez-vous confirmé !</h2>
			...
			<div className="animate-pulse">
				<div className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-4 py-2">
					...
					<span className="text-xs text-emerald-700">Redirection en cours...</span>
				</div>
			</div>
		</div>
	);
	...
	return (
		<>
			<Helmet>
				...
				<meta name="description" content="Prenez rendez-vous avec un conseiller Paname Consulting" />
				<link rel="canonical" href="https://panameconsulting.vercel.app/rendez-vous" />
					...
					{success ? (
						...
						>
							<div className="border-b border-gray-100 bg-linear-to-r from-sky-500 to-sky-600 px-6 py-4">
								<h1 className="text-xl font-bold text-white">Prendre un rendez-vous</h1>
