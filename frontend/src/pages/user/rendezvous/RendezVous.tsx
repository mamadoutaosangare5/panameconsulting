...
const RendezVous = () => {
  ...
  // Utilisation du hook useRendezvous
  const {
    createRendezvous,
    checkAvailability,
  ...
  // Charger les dates disponibles via le service
  const fetchAvailableDates = useCallback(async (): Promise<void> => {
    try {
      const dates: AvailableDate[] = await rendezvousService.getAvailableDates();
  ...
  const isStepValid = (step: number): boolean => {
    switch (step) {
      ...
      case 2:
        ...
        if (formData.niveauEtude === 'Autre' && !formData.niveauEtudeAutre?.trim())
          return false;
