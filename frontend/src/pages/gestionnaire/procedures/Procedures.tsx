...
import type { ProcedureResponseDto, ProcedureStatus } from "../../../types/procedures.types";
import { Search, Edit2, Trash2, Eye, Clock, User, CheckCircle, AlertCircle, XCircle } from "lucide-react";
...
const Procedures = () => {
	...
	const filteredProcedures = useMemo(() => {
		...
		if (statusFilter !== "tous") {
			filtered = filtered.filter((procedure: ProcedureResponseDto) => procedure.statut === statusFilter);
		}

		// Filtrer par destination
		if (destinationFilter !== "tous") {
			filtered = filtered.filter(
				(procedure: ProcedureResponseDto) => procedure.effectiveDestination === destinationFilter,
			);
		}
	...
	const destinations = useMemo(() => {
		const uniqueDestinations = [...new Set((procedures || []).map((p) => p.effectiveDestination).filter(Boolean))];
	...
	// Obtenir le nombre de procédures par statut
	const getStatusCount = (status: ProcedureStatus) => {
		return (procedures || []).filter((p) => p.statut === status).length;
	};
		...
		<>
			...
			<div className="p-4 md:p-6 lg:p-8">
				...
				<div className="mb-6">
					...
					<p className="text-gray-600">Gérez toutes les procédures d'immigration</p>
				</div>
									...
									<tr>
										<td colSpan={6} className="px-4 py-12 text-center">
											<div className="text-gray-500">
												...
												<p className="text-lg font-medium mb-2">Aucune procédure trouvée</p>
										...
										<tr key={procedure.id} className="hover:bg-gray-50">
											<td className="px-4 py-4">
												<div>
													...
													<div className="text-sm text-gray-500">{procedure.email}</div>
												</div>
											</td>
											<td className="px-4 py-4">
												<div className="text-gray-900">{procedure.destination}</div>
											</td>
											<td className="px-4 py-4">{getStatusBadge(procedure.statut)}</td>
											<td className="px-4 py-4">
												...
												<div className="flex items-center gap-2">
													<div className="flex-1 bg-gray-200 rounded-full h-2">
														<div
															...
															style={{ width: `${procedure.progress || 0}%` }}
														></div>
													</div>
