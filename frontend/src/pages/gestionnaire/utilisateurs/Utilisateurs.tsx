...
import type { CreateUserParams, UpdateUserParams } from "../../../types/user.types";
	...
	<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
		<div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm border border-slate-200 overflow-hidden">
			<div className="p-6">
				...
				<h3 className="text-center font-serif text-lg text-slate-900 mb-1">Supprimer l'utilisateur</h3>
...
const CreateModal = ({
	...
	const field = (label: string, key: keyof CreateForm, type = "text", placeholder = "") => (
		<div>
		...
		<div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-8 sm:items-center bg-slate-900/40 backdrop-blur-sm overflow-y-auto">
			<div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-slate-200">
				<div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-slate-100">
					<h3 className="font-serif text-lg text-slate-900">Nouvel utilisateur</h3>
				...
				<div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
					...
					<div className="sm:col-span-2">{field("Mot de passe", "password", "password")}</div>
				</div>
		...
		<div className="fixed inset-0 z-50 flex items-start justify-center p-4 pt-8 sm:items-center bg-slate-900/40 backdrop-blur-sm overflow-y-auto">
			<div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-slate-200">
				...
				<div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
					...
					<div className="sm:col-span-2">{field("Téléphone", "telephone", "tel")}</div>
				</div>
				<div className="flex gap-3 px-6 pb-6">
					<button
						onClick={onClose}
...
const Utilisateurs = () => {
	...
	const load = useCallback((p: number) => fetchUsers({ page: p, limit: LIMIT }), [fetchUsers]);
	...
	return (
		<>
			<Helmet>
				...
				<meta name="description" content="Gérez les utilisateurs de Paname Consulting" />
			...
			<div className="min-h-screen p-4 sm:p-6 font-sans">
				...
				<div className="max-w-6xl mx-auto mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
					<div>
						<div className="flex items-center gap-2 mb-0.5">
							...
							<h1 className="text-2xl sm:text-3xl font-serif text-slate-900 font-normal">Utilisateurs</h1>
						</div>
						<p className="text-slate-500 text-sm">
							{userList ? (
								<>
									<span className="font-semibold text-slate-700">{userList.total}</span> utilisateur
					...
					<div className="max-w-6xl mx-auto mb-6 sm:mb-8">
						<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
							...
							<div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
								...
								<p className="text-gray-600 text-xs sm:text-sm">Total Utilisateurs</p>
							...
							<div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
								...
								<p className="text-gray-600 text-xs sm:text-sm">Utilisateurs Actifs</p>
								<div className="mt-2 flex items-center text-xs text-gray-500">
							...
							<div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
								...
								<p className="text-gray-600 text-xs sm:text-sm">Utilisateurs Inactifs</p>
								<div className="mt-2 flex items-center text-xs text-gray-500">
									<AlertCircle className="w-3 h-3 mr-1" />
									{statistics.totalUsers
										? Math.round((statistics.inactiveUsers / statistics.totalUsers) * 100)
										: 0}
									% du total
								</div>
							</div>
							...
							<div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
								...
								<p className="text-gray-600 text-xs sm:text-sm">Administrateurs</p>
								<div className="mt-2 flex items-center text-xs text-gray-500">
									<Crown className="w-3 h-3 mr-1" />
									{statistics.totalUsers
										? Math.round((statistics.adminUsers / statistics.totalUsers) * 100)
										: 0}
									% du total
								</div>
							</div>
				...
				<div className="max-w-6xl mx-auto mb-4 flex gap-3">
					...
					<button
						...
						title="Actualiser"
					>
						<RefreshCw size={15} className={loading.list ? "animate-spin" : ""} />
					</button>
				</div>
				...
				<div className="max-w-6xl mx-auto bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
					...
					{loading.list && !userList && (
						<div className="flex flex-col items-center justify-center py-20 gap-3">
							...
							<span className="text-slate-400 text-sm">Chargement des utilisateurs…</span>
						</div>
					)}
							...
							<div className="hidden sm:block overflow-x-auto">
								<table className="w-full text-sm">
									...
									<tbody className="divide-y divide-slate-50">
										{filtered.map((user) => (
											<tr key={user.id} className="hover:bg-sky-50/40 transition-colors group">
												{/* Utilisateur */}
												...
												<td className="px-4 py-3.5 hidden lg:table-cell">
													...
													<span className="text-xs text-slate-400 ml-1">connexions</span>
												</td>
										...
										<div className="flex-1 min-w-0">
											<div className="flex items-center justify-between gap-2 mb-1">
												...
												<div className="flex gap-1 flex-shrink-0">
													...
													>
														{user.isActive ? <Shield size={13} /> : <UserCheck size={13} />}
													</button>
													<button
														onClick={() => openEdit(user)}
														className="p-1.5 rounded-lg text-slate-400 hover:text-sky-500 hover:bg-sky-50 transition-colors"
