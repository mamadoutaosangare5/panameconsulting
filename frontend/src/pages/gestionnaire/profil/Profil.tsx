...
const Profil = () => {
  const { profile, loading, patchAdminProfile, patchUser } = useUser();
  ...
  const handleSave = async () => {
    ...
    if (profile.role === "ADMIN") {
      const params: UpdateAdminProfileParams = {
        firstName: formData.firstName,
        lastName: formData.lastName,
        telephone: formData.telephone,
      };
      const result = await patchAdminProfile(params);
      ...
      const result = await patchUser(profile.id, params);
      if (result) setIsEditing(false);
    }
  };
  ...
  const isSaving = loading.updateProfile;
        ...
        <div className="max-w-3xl mx-auto flex flex-col gap-5">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 text-center shadow-sm">
            <div className="flex justify-center mb-4">
              <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-gradient-to-br from-sky-400 to-indigo-500 flex items-center justify-center font-serif text-2xl sm:text-3xl text-white tracking-wider shadow-lg shadow-sky-200">
                {initials || "?"}
              </div>
            </div>

            <div className="flex items-center justify-center gap-2 mb-1">
              <span className="w-2 h-2 rounded-full bg-emerald-400 shadow-sm shadow-emerald-200 flex-shrink-0" />
              ...
              <div>
                ...
                {isEditing ? (
                  ...
                  <div className="flex items-center gap-2.5 py-2 border-b border-slate-100">
                    <span className="w-7 h-7 bg-sky-50 rounded-lg flex items-center justify-center text-sky-400 text-xs font-bold flex-shrink-0">
                      {profile?.firstName?.[0] ?? "?"}
                    </span>
